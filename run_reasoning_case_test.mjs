...
function dumpRequestBody(name, body) {
  fs.mkdirSync(REQUEST_DUMP_DIR, { recursive: true });
  const path = `${REQUEST_DUMP_DIR}/${SCENARIO}_${name}.json`;
  fs.writeFileSync(path, JSON.stringify(body, null, 2));
  return path;
}

function dumpModelResponse(name, payload) {
  fs.mkdirSync(RESPONSE_DUMP_DIR, { recursive: true });
  const path = `${RESPONSE_DUMP_DIR}/${SCENARIO}_${name}.json`;
...
function analyze(text) {
  ...
  return {
    ...
    englishWordsSample: [...new Set(englishWords)].slice(0, 30),
...
function parseSseLine(line, acc) {
  ...
  if (delta.tool_calls || message.tool_calls) {
    acc.toolCalls.push(...(delta.tool_calls ?? message.tool_calls));
  }
...
function parseNonStream(raw, acc) {
  ...
  acc.reasoning =
    ...
    "";
  acc.content = choice.message?.content ?? choice.delta?.content ?? "";
  acc.toolCalls = choice.message?.tool_calls ?? choice.delta?.tool_calls ?? [];
  acc.finishReason = choice.finish_reason ?? "";
}

async function callVariant(name, makeBody) {
  const body = makeBody();
  const requestBodyPath = dumpRequestBody(name, body);
  ...
  const responseDump = {
    ...
    rawPreview: !response.ok || parseError ? raw.slice(0, 1200) : undefined,
    reasoning: acc.reasoning,
    content: acc.content,
    toolCalls: acc.toolCalls,
    rawResponse: !response.ok || parseError ? raw : undefined,
  };
  const responsePath = dumpModelResponse(name, responseDump);
...
const results = [];
for (const [name, makeBody] of variants) {
  console.error(`Running ${name}...`);
  if (process.env.DUMP_ONLY === "1") {
    const body = makeBody();
    results.push({ name, scenario: SCENARIO, requestBodyPath: dumpRequestBody(name, body) });
    continue;
  }
  try {
