...
const REQUEST_DUMP_DIR =
  ...
  "test_request_bodies/after_read_skill_failures_first_turn_cases";
const RESPONSE_DUMP_DIR =
  process.env.RESPONSE_DUMP_DIR ||
  "test_model_responses/after_read_skill_failures_first_turn";
const OUTPUT_PREFIX =
  process.env.OUTPUT_PREFIX || "after_read_skill_failures_first_turn";
...
const CHANGE_DEFINITIONS = {
  ...
  2: {
    name: "first_turn_user_json_instruction_chinese",
    description: "将首次用户请求中的 JSON 输出格式说明从英文改成中文。",
  },
};
...
function applyChanges(body, chineseBody, changes) {
  if (changes.includes(1)) {
    body.messages[0].content = chineseBody.messages[0].content;
  }

  const userMessage = changes.includes(2)
    ? clone(chineseBody.messages[18])
    : clone(readJson(BASELINE_BODY).messages[18]);
  body.messages = [body.messages[0], userMessage];
}

function buildBody(caseInfo) {
  const baselineBody = readJson(BASELINE_BODY);
  const chineseBody = readJson(CHINESE_BODY);
  const body = clone(baselineBody);
  applyChanges(body, chineseBody, caseInfo.changes);
  return body;
}
